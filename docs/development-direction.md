...
## 다음 단계
1. 근무표 배포 양식 정의를 확정하고 월간 배포/내보내기 흐름과 맞춘다.
2. 운영 관리의 공휴일, 요율, 사용자, 양식에 대한 저장/수정/동기화 범위를 확정하고 bridge를 보강한다.
3. 대시보드 집계와 empty state를 실데이터 기준으로 정리하고 demo fallback 의존을 줄인다.
4. Phase 3-5의 수동 검증과 `npm run test` 종료 이슈 정리를 마친 뒤 Phase 6 패키징/최종 문서 정리로 넘어간다.
