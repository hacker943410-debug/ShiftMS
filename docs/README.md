...
- 대상 버전: `0.3.2` 작업 중 (`0.3.2` 설치본 생성 및 smoke 검증 기준)
- 현재 단계: `0.3.2` 패키징 검증 완료, 양식 관리 개편/대시보드 기간 선택 UI/운영 관리 사이트 명 가이드 보강
- 자동 검증 마지막 재확인: `2026-04-14`
- 최근 반영 변경: `2026-04-14` `ShiftMgmt-Setup-0.3.2-x64.exe` 생성 및 packaged/installer smoke 검증, 메뉴별 사용자 설명서 추가, 대시보드 기간 직접 지정 월 선택 팝오버 통일, 운영 관리 `사이트 명 관리` 가이드 추가
