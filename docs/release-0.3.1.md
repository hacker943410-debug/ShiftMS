...
- 실적 파일 파싱은 배포/저장된 월간 근무표를 기준 데이터로 삼아 법정휴일근무와 대체근무 시간을 복원하는 운영 흐름으로 확정한다.
...
3. `DB업데이트` 복원 입력은 JSON 백업 파일로 제한되며, Access/Excel 파일은 복원 대상으로 선택하지 않는다.
4. 실적 파일 파싱 전에 해당 월/근무지 근무표가 배포 또는 저장되어 있지 않으면 법정휴일근무/대체근무 시간 복원이 제한되어 0시간 경고 행이 발생할 수 있다.
