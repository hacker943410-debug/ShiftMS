...
## 요약
0.4.14는 인력관리 목록 조회 방식과 직원 데이터 기준을 정리하고, 2026-04 품의서/별첨1 양식과 별첨1 시급 표기, 수당 올림 계산, 품의서 체크박스/대상자 카운트를 보정한 패치입니다.
...
14. 품의서 Excel은 일반 지급, 퇴사자 조기 지급, 총 합계, 지급 요청일/세부내역 구조를 유지합니다.
15. 별첨1 Excel은 `별첨1` 시트만 생성하고, 퇴사자 조기 지급 대상이 없어도 `해당 없음` 행과 새 샘플 기준 정적 계산 안내를 출력합니다.
