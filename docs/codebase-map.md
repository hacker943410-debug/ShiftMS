...
| **업데이트는 사용자 데이터를 건드리지 않는다** | 설치본은 앱 파일만 교체한다. `build/installer.nsh`의 설치 후 되돌리기(`build/installer-update-data.ps1`)는 **빠진 파일만 채우고** 라이브 폴더를 지우거나 덮어쓰지 않는다. 여기에 삭제를 다시 넣으면 무인 자동업데이트에서 **안내 없이 DB가 날아간다**. `nsis.deleteAppDataOnUninstall`도 켜지 말 것. **0.5.7 이후 세 가지가 더 붙었다** — ① 라이브에 **내용이 든 DB 로그(`-wal`)** 가 남아 있으면 지우지 않고 `...-wal-unrestored-<시각>`으로 **이름만 바꿔 그 자리에 둔다** ② 그때 종료코드가 0이 아니라 **3**(= 끝났지만 한 개를 치워 뒀다)이고, 안전 복사본도 **한 번 더 남긴다** ③ 못 읽는 폴더·바꿔 끼울 수 없는 파일은 이제 **0으로 조용히 끝나지 않고 실패로 멈춘다**(그래도 지운 것은 없다) → `docs/maintainer-guide.md` 패키징 메모 |
