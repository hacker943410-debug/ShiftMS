...
| **업데이트는 사용자 데이터를 건드리지 않는다** | 설치본은 앱 파일만 교체한다. `build/installer.nsh`의 설치 후 되돌리기(`build/installer-update-data.ps1`)는 **빠진 파일만 채우고** 라이브 폴더를 지우거나 덮어쓰지 않는다. 여기에 삭제를 다시 넣으면 무인 자동업데이트에서 **안내 없이 DB가 날아간다**. `nsis.deleteAppDataOnUninstall`도 켜지 말 것. **0.5.7 이후 세 가지가 더 붙었다** — ① 라이브에 **내용이 든 DB 로그(`-wal`)** 가 남아 있으면 지우지 않고 `...-wal-unrestored-<시각>`으로 **이름만 바꿔 그 자리에 둔다** ② 그때 종료코드가 0이 아니라 **3**(= 끝났지만 치워 둔 것이 있다 — 개수는 한 개로 정해져 있지 않고, 로그 이름 자리에 있던 **폴더**도 같은 식으로 이름만 바뀐다)이고, 안전 복사본도 **한 번 더 남긴다** ③ **백업 쪽** 못 읽는 폴더와 바꿔 끼울 수 없는 파일은 이제 **0으로 조용히 끝나지 않고 실패로 멈춘다**(그래도 라이브 폴더를 비우거나 갈아엎지는 않는다). **라이브 쪽** 못 읽는 폴더는 실패가 아니라 종료코드 **4** 로 끝날 수 있다 — 채워 넣기는 끝까지 돌았지만 "더 채울 게 없다"고 단정만 못 하는 상태로 복사본을 남긴다. **다만 "복사본이 그 폴더 안까지 들고 있으면 반드시 실패" 는 아니다**(그렇게 적혀 있었는데 틀렸다): 폴더가 **목록만 거부하고 이름을 대면 답해 주면** 그 안까지 채운 뒤 그대로 `4` 다(실측). **"이건 이미 있나" 라는 물음 자체가 거부되는** 폴더일 때만 거기서 멈춰 실패로 끝난다(그 멈춤은 예전부터 그랬다) → `docs/maintainer-guide.md` 패키징 메모 · `docs/open-defects.md` 5장 |
