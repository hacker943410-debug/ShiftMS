...
| 인력 관리 | `screens/WorkforceManagementScreen.tsx` | `screens/workforce/` (시급 구간 판정 `screens/workforce/wage-rate-timeline.ts` · 시급 일괄 업데이트 모달의 상태·동작 전부 `screens/workforce/useWageBulkUpdate.ts` · 그 열 매핑 모델 `screens/workforce/wage-bulk-preview-basis.ts`) | `employee-storage-service.ts` · `employee-history-service.ts` · `workforce-wage-bulk-update-service.ts` |
