...
| **업데이트는 사용자 데이터를 건드리지 않는다** | 설치본은 앱 파일만 교체한다. `build/installer.nsh`의 설치 후 되돌리기는 **빠진 파일만 채우고** 라이브 폴더를 지우거나 덮어쓰지 않는다. 여기에 삭제를 다시 넣으면 무인 자동업데이트에서 **안내 없이 DB가 날아간다**. `nsis.deleteAppDataOnUninstall`도 켜지 말 것 → `docs/maintainer-guide.md` 패키징 메모 |
