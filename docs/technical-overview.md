...
## 문서 역할
- 이 문서는 `교대근무관리시스템 v0.4.14`의 개발 언어, 로컬 백엔드 구성 방식, 저장소, 파일 연동, 구현 원칙을 코드 기준으로 정리한 기술 개요서다.
...
### DB 업데이트와 백업
- `DB업데이트`는 JSON 백업 `.json` 또는 Access DB `.accdb` 파일을 입력으로 받는다.
- Excel 파일은 운영자가 임의로 핸들링할 수 있으므로 복원 입력에서 제외한다.
...
- 기본 품의서/별첨1은 2026-04 수정본 파일을 사용하며, 설치본 리소스 fallback도 같은 파일명을 기준으로 한다.
- 별첨1 2026-04 compact writer는 최종 workbook에서 `별첨1` 외 시트를 제거하고, 일반 지급 표와 퇴사자 조기 지급 표를 분리 출력한다.
- 별첨1 하단 안내는 새 샘플의 정적 안내를 사용하되 금액 계산은 `roundUpWon` 기준 앱 결과값을 쓴다.
