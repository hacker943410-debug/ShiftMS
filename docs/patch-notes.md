...
## V0.5.8
- 기준일: `2026-09-16`
- 성격: 인력·시급 이력, 승인 재분석, 설치 데이터 안전성 보강
- 현재 상태: GitHub Release 게시 완료(tag `v0.5.8`, 자동업데이트 `latest.yml` 포함)

### 핵심 변경
- 새 시급 적용일을 직접 선택하고 시작일·종료일·금액·사유, 공백·겹침을 구간 표에서 확인한다.
- 특정 시급 행을 정정하거나 사유를 입력해 삭제할 수 있고, 삭제 전 값과 작업자를 영구 감사 이력에 남긴다.
- 같은 시작일의 최신 시급을 삭제하면 다시 적용될 정확한 금액을 경고하고 main 프로세스에서 확인값을 재검증한다.
- 인력·근무표·정책·시급 변경 후 재분석에 실패한 파일은 과거 분석값으로 승인할 수 없다.
- 재입사·퇴사일 정정·월중 근무지/조 이동과 근무지 반려 실패 보상을 이력·트랜잭션 기준으로 보강했다.
- 설치 업데이트 복원 중 경로 충돌, 잠긴 마커, 링크·정션, 읽기 실패가 있으면 사용자 데이터를 지우지 않고 안전하게 중단한다.

### 검증
- `npm run test` — `178 files / 1,267 tests`
- `npm run typecheck`, `npm run build`, `npm run release:check`
- `npm run lint` — 오류 0개, 기존 경고 25개
- `npm run validate:map`, `npm run validate:harness`, `node scripts/validate-structure.mjs`
- 임금 이력 UI 캡처 — `11/11`
- `npm run release:verify-package`
- GitHub Release 필수 자산 4개와 SHA-256 일치 확인
- 공개 설치본 재설치 및 기존 데이터 보존 smoke 통과
...
- 실적 엔트리에 조 정보를 저장해 재조회와 재승인 흐름에서도 조별 정렬이 유지된다.
- 반려 상태의 승인대기 파일은 파일 크기와 수정 시간이 같아도 기존 DB 분석 결과를 재사용하지 않고 다시 파싱한다.
- 변경/대체 표에서 `원근무자 -> None`으로 입력하면 같은 일자의 원근무자 법정휴일 실적을 생성하지 않는다.
- 원근무자 셀 자체가 `None`이거나 같은 슬롯 변경 후 셀이 `None`인 기존 취소 조건은 그대로 유지한다.
- 로그인 비밀번호 5회 실패 계정 잠금을 제거하고, 실패 횟수는 감사 확인용 기록으로 유지한다.

### 운영 참고
- 이미 수동 이동된 승인대기 파일은 패치 적용 후 해당 월 실적관리 조회 또는 새로고침으로 현재 Excel 기준 재분석된다.
- 향후 승인/반려는 폴더 직접 이동이 아니라 앱의 승인/반려 흐름을 사용해야 승인 이력과 파일 상태가 함께 맞는다.

### 검증
- `npm run typecheck`
- `npx vitest run --maxWorkers=1 --minWorkers=1 schedule-return-performance-parser.test.ts` — `26 tests`
- `npx vitest run --maxWorkers=1 --minWorkers=1 performance-management-service.test.ts` — `18 tests`
...
- 기본 인력 목록에서는 숨김 인력을 제외하지만, 실적 계산 경로는 근무일 기준으로 숨김 인력과 과거 배치 이력을 조회한다.
- 반환 근무표 파서는 동명이인 후보를 근무일의 과거 근무지 배치 이력으로 먼저 좁혀 직무이동 후에도 과거월 사번/시급을 찾는다.
- 월근무표 자동 복원은 현재 근무지가 아니라 대상 월의 근무지 배치 이력으로 복원 인력을 찾는다.
- 기존 버전에서 직원이 물리 삭제되어 빈 사용자 근무표처럼 보이는 경우, 직원/시급/배치 이력을 백업 또는 재등록으로 복원하면 배포 근무표 기반 재복원이 가능하도록 대상 판정을 열었다.

### 운영 참고
- 이미 물리 삭제된 직원은 패치만으로 자동 복구할 수 없다. DB 백업 복원 또는 동일 사번/시급/배치 이력 재등록 후 재파싱해야 한다.
- 임의 시간/시급 입력 지급은 감사 이력과 승인 통제가 필요한 별도 보정 기능으로 분리한다.

### 검증
- RED 확인: 삭제 후 과거 실적 0분/시급 없음, 복원 0건 실패 상태 재현
- `npx vitest run --maxWorkers=1 --minWorkers=1 src/main/services/employee-storage-service.test.ts src/main/services/schedule-return-performance-parser.test.ts src/main/services/performance-file-intake-service.test.ts`
- `npm run typecheck`
...
- 기준일: `2026-06-12`
- 성격: 실적관리 계산·표시 안정화
- 현재 상태: GitHub Release 게시 완료(tag `v0.4.25`, 자동업데이트 `latest.yml` 포함)

### 핵심 변경
- 근무지 관리 상세 보기에서 Cycle 근무시간이 저장 step 순서가 아니라 canonical slot 기준 `1근`, `2근`, `3근` 순서로 표시된다.
- 실적관리 새로고침 버튼은 변경 없는 반환 파일도 다시 파싱해 저장된 월근무표 시간이 바뀐 경우 총근무/연장 시간 계산을 갱신한다.
- 근무표 또는 반환 근무표에서 `None`으로 지정한 칸은 법정휴일, 대체, 연장 실적 대상에서 제외하고 실적관리 목록에도 표시하지 않는다.
- 실적관리 필터 순서를 `연도`, `월`, `근무지명`, `조회구분`, `근로유형`으로 바꾸되, 각 필터 박스 폭은 기존 값과 동일하게 유지했다.
- 실적관리 테이블 밀도를 높여 데스크톱 화면에서 불필요한 좌우 스크롤을 줄이고, 좁은 화면에서는 가로 스크롤 fallback을 유지한다.
- 품의서 템플릿 등록 단계에서 신형/구형 배치를 판정하고, 구형 템플릿은 저장을 차단해 문서 생성 오류를 예방한다.

### 검증
- `npm run test` — `130 files / 630 tests`
- `npm run build`
- `git diff --check`
- `npm run release:check`
- `npm run release:publish`
...
## V0.4.24
- 기준일: `2026-06-12`
- 성격: 0.4.23 근무표 복구 후속 안정화(하드닝)
- 현재 상태: GitHub Release 게시 완료(tag `v0.4.24`, 자동업데이트 `latest.yml` 포함)

### 핵심 변경
- 복구 시 시작=종료 같은 0길이(퇴화) 근무시간을 차단해 다운스트림 약 24시간 유령 근무 계산을 막는다. 초(SS) 범위 검증과 자정 경계 분류(16:00–00:00=석간, 00:00–08:00=야간)도 보정했다.
- 패턴에서 시간을 찾지 못한 근무 칸은 0분 행으로 저장하지 않고 건너뛴 뒤 “근무 시간을 확인하지 못해 제외했습니다” 경고로 노출한다(조용한 0분 방지).
- 부분만 복구된 달은 `system-restore-partial`로 태깅해 재복구 대상으로 유지하고, 옛 버전이 남긴 미완성 근무표(빈 시간·0건·중복 행·숨겨진 퇴사자 null 행)도 기존 id를 재사용해 교체·치유한다. 결과가 같으면 다시 저장하지 않는 멱등 복구로 매 부팅 재처리를 막는다.
- 수동 근무표와 이미 정상인 근무지의 근무시간은 자동 복구가 덮어쓰지 않는다.

### 검증
- 적대적 검증 5라운드 교차검증(`docs/review-0.4.24-hardening.md`), R5=조건부 승인 가능
- 실데이터 end-to-end: 동작국사 A/B/C 치환 슬롯 홍길동→유성 → 복구 후 유성 420분(135,450원)·휴일 660분(237,600원), 복구 전 0분 → 정상화
- `npm run typecheck` (0)
- `npm run test` — `130 files / 625 tests`
- `npm run build`
- `npm run release:check`

## V0.4.23
- 기준일: `2026-06-11`
- 성격: 근무표 복구 시 듀티코드 불일치로 인한 휴일·대체 근로시간 0분 수정
- 현재 상태: GitHub Release 게시 완료

### 핵심 변경
- 근무표 복구 시 근무표 칸을 주간/석간/야간 위치(D/E/N)로 읽으면서 근무시간은 패턴 근무코드로만 찾던 탓에, 패턴이 `A/B/C` 등 D/E/N과 다른 코드를 쓰는 근무지(예: SKB동작국사 5조3교대)는 복구 항목에 시간이 비어 법정공휴일/대체 근로시간이 0분(수당 0원)으로 계산됐다.
- 복구 시 패턴 근무의 시작 시각으로 주간(D)/석간(E)/야간(N)을 분류해 D/E/N 칸에 실제 근무시간을 채운다. 이미 D/N 코드를 쓰던 근무지(판교DC 등)는 기존 시간을 그대로 유지한다.
- 단, 해당 월의 근무표가 시스템에 존재해야 정상 계산된다(배포 근무표 원본이 없어 복구 불가한 달은 별도 사안).

### 검증
- 실데이터 end-to-end: 복구 → 주간 `09:00-18:00` 채움 → 휴일 480분 → 수당 191,868원, 판교DC 보존
- `npm run typecheck` (0)
- `npm run test` — `129 files / 602 tests`
- `npm run build`
- `npm run release:check`

## V0.4.22
- 기준일: `2026-06-11`
- 성격: 별첨1 대량 항목 행 삽입 병합 충돌 수정, 폴더 감시 부분복사 파싱 방지
- 현재 상태: GitHub Release 게시 완료

### 핵심 변경
- 별첨1 정규 요약이 템플릿 용량(10행)을 초과해 `spliceRows`로 행이 삽입되기 전에 선지급 헤더 영역(19-21행)의 잔존 병합을 먼저 해제해, 옮겨지지 않은 `A19:A21` 병합이 새 헤더 `A21:A23`과 충돌하던 `Cannot merge already merged cells` 출력 실패를 제거했다.
- 헤더 병합 직전에도 대상 범위 잔존 병합을 정리해 행 삽입·삭제 양쪽 경로를 보호한다(0.4.21은 행 제거 경로만 다룸).
- 폴더 감시에 `awaitWriteFinish`(2초 안정화)를 적용해 네트워크/폴더 복사 도중의 부분 저장본을 파싱하지 않는다. 기존 파일은 종전대로 startup 동기화가 처리한다.

### 검증
- `npm run typecheck` (0 오류)
- `npm run test` — `128 files / 596 tests`
- fail-on-old 실증 — 별첨1 splice 前 언머지 비활성화 시 대량(12 정규 + 3 선지급) 출력이 `A21:A23 ↔ A19:A21` 충돌로 실패, 수정 시 통과
- `npm run build`
- `npm run release:check`
