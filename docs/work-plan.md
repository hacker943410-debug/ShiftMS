...
- 인력 상세 화면은 이제 배정 이력과 시급 이력을 renderer에서 직접 저장/종료할 수 있고, 저장 실패 시 예외 메시지도 화면에서 확인할 수 있다.
- 근무지 관리 화면은 이제 실제 근무지/패턴 저장, 패턴 시작일과 팀 수 관리, 그리고 Step 2 조별 인력 배정을 실제 employee assignment bridge로 처리한다.
- 근무표 관리 화면은 이제 실제 월간 스케줄 초안을 shared domain 유틸에서 생성하고, 저장된 근무표 기준으로 미리보기, 엑셀 내보내기, 배포 승인 이력을 같은 화면에서 확인한다.
...
**계획 상태**: 진행 중
**다음 작업**: Vitest 전체 runner 종료 이슈를 정리하고 Phase 6 전체 메뉴 스모크와 대시보드/최종 운영 플로우 점검에 착수한다.
**현재 blocker**: 없음
