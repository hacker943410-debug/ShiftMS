...
**계획 상태**: 진행 중
**다음 작업**:
1. 운영 관리 페이지 메뉴를 역할별로 분할하고, 공휴일 -> 요율 -> 사용자 -> 양식 순서로 메뉴별 패치를 진행한다.
2. 운영 관리 메뉴 분할 완료 후 실적 관리 페이지 패치를 진행한다.
3. 실적 관리 패치 이후 수당 관리 페이지 패치를 진행한다.
**보류 작업**:
- `npm run test` 종료 시 발생하는 `ERR_IPC_CHANNEL_CLOSED` 해결
- 운영 관리의 공휴일, 요율, 사용자 탭 저장/수정/동기화 범위 보강
