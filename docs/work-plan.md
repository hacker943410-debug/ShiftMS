...
**계획 상태**: 진행 중
**다음 작업**:
1. 실적 관리 메뉴를 계속 패치하고, 경로 설정/필터/대기열/상세 흐름을 실제 운영 기준으로 다듬는다.
2. 실적 승인 이후 이어지는 수당 산출 로직을 구현하고 승인 스냅샷과 계산 결과를 같은 규칙으로 맞춘다.
3. 수당 관리 페이지를 계산 결과 확인, 산출 실행, 문서 출력 기준으로 이어서 패치한다.
**보류 작업**:
