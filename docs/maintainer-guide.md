...
- **되돌리기가 끝나지 않은 백업은 지우지 않는다.** 다음 백업 때 `...-unrestored-<시각>` 으로 **이름만 바꿔 옆에 둔다**(최근 3개 유지). 그 백업이 유일한 사본일 수 있어서다. `installer.nsh` 에서 이 폴더를 `RMDir` 하지 말 것.
...
- **백업에 로그(`-wal`/`-shm`)만 있고 DB 본체가 없으면 그 로그를 넣지 말고 오류로 끝낸다.** 백업은 못 읽는 파일을 한 개씩 건너뛰므로 이런 반쪽 백업이 생길 수 있다. 로그만 되돌리면 SQLite 가 열지 못하는 폴더가 남는데, 그대로 성공 처리하면 **백업까지 지우고 끝난다.** 라이브 본체가 살아 있는 경우는 애초에 로그를 넣지 않으므로 이 오류가 나지 않는다.
